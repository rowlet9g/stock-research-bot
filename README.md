...
로그인한 ChatGPT 플랜의 Codex 사용량 한도를 사용합니다. 실시간 웹 검색으로
OpenDART·SEC·회사 IR·ETF 운용사·공식 거시지표를 우선 조사하며, 별도 Platform
API 키는 필요하지 않습니다.
...
ChatGPT로 인증된 Codex CLI의 실시간 웹 리서치, 응답 파일 저장과 이메일 미리보기를 한 번에
수행합니다. `-send`를 붙인 경우에만 기존 SMTP 설정으로 전송합니다. 별도 ChatGPT
창, 프롬프트 복사 또는 메모장 작업은 필요하지 않습니다. 이메일은 날짜와 시간대만
메타데이터로 표시하고 행동 중심 일반 텍스트로 작성합니다. 모든 활성 종목에
추가 매수·보유·부분 매도·전량 매도 중 하나의 의견을 요구하고, 신규 후보 2~4개와
종목별 1차 출처로 분류된 항목을 포함하지 않은 구조화 응답은 거부합니다. 인증과 실행 제한,
추적 해시 및 수동 예비 경로는
