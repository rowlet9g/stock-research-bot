...
- 심각도별 경고, 관찰, 정보 건수와 주제별 사실·진단·확인 질문
- 종목별 반복 알림을 최대 6개 포트폴리오 점검 주제로 그룹화
...
답변을 자동으로 가져오는 기능은 아닙니다. `portfolio-brief`가 생성한 프롬프트를
ChatGPT Plus에서 해석하거나, 후속 읽기 전용 API와 GPT Actions를 연결해야
대화형 보유·리밸런싱 검토가 가능합니다.
...
같은 유형의 종목별 사실은 해당 섹션의 `관측` 목록으로 합치고 공통 확인 질문은
중복을 제거해 최대 4개만 표시합니다. 원본 알림은 삭제하지 않으며 JSON 출력의
`report.alerts`에 그대로 보존합니다.
...
- 2026-07-29 실제 SMTP 계정의 합성 알림과 pending 알림 전송을 검증했습니다.
